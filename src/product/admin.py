from django.contrib import admin
from product.models import ProductUnit,ProductVariant,ProductVariantAttribute,ProductVariantReview,CountryWiseProductVariant


class ProductUnitAdminColumn(admin.ModelAdmin):
    def get_list_display(self, request):
        return [field.name for field in self.model._meta.fields]
    
    # Make all fields clickable
    def get_list_display_links(self, request, list_display):
        return list_display 
admin.site.register(ProductUnit,ProductUnitAdminColumn)

class ProductVariantAdminColumn(admin.ModelAdmin):
    def get_list_display(self, request):
        return [field.name for field in self.model._meta.fields]
    
    # Make all fields clickable
    def get_list_display_links(self, request, list_display):
        return list_display 

admin.site.register(ProductVariant,ProductVariantAdminColumn)

class ProductVariantAttributeAdminColumn(admin.ModelAdmin):
    def get_list_display(self, request):
        return [field.name for field in self.model._meta.fields]
    
    # Make all fields clickable
    def get_list_display_links(self, request, list_display):
        return list_display 
admin.site.register(ProductVariantAttribute,ProductVariantAttributeAdminColumn)

class ProductVariantReviewAdminColumn(admin.ModelAdmin):
    def get_list_display(self, request):
        return [field.name for field in self.model._meta.fields]
    
    # Make all fields clickable
    def get_list_display_links(self, request, list_display):
        return list_display 
admin.site.register(ProductVariantReview,ProductVariantReviewAdminColumn)

class CountryWiseProductVariantAdminColumn(admin.ModelAdmin):
    def get_list_display(self, request):
        return [field.name for field in self.model._meta.fields]
    
    # Make all fields clickable
    def get_list_display_links(self, request, list_display):
        return list_display 
admin.site.register(CountryWiseProductVariant,CountryWiseProductVariantAdminColumn)

